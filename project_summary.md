...
## 📱 사용자 편의성 및 모바일 대응
* **모바일 기기 접속 권장 팝업**: 
  - 최초 진입 시, 너비 768px 미만 또는 모바일 `userAgent` 감지 시 경고 모달을 띄워 PC/태블릿 접속을 권장합니다.
  - 글래스모피즘(반투명 블러 오버레이)과 클로드 파스텔 톤 테두리, 페이드인 스케일 업 애니메이션이 적용되어 있습니다.
  - 팝업 내 우회 버튼(계속 진행하기)을 통해 모바일에서도 계속 진입할 수 있도록 예외 처리가 반영되어 있습니다.
...
* **`src/App.jsx`**:
  ...
  - 모바일 감지 조건 상태(`showMobileModal`) 제어 및 모달 팝업 연동.
...
* **`src/index.css`**:
  ...
  - 모바일 권장 모달 팝업 애니메이션 및 CSS 스타일 적용.
